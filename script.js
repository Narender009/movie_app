...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateSessionStorage() {
        ...
        if (currentGenreType) {
            sessionStorage.setItem('currentGenreType', currentGenreType);
        } else {
            sessionStorage.removeItem('currentGenreType');
        }
    }
    ...
    function redirectToDetailsPage(id, type) {
        let detailType = type;
        if (type === 'bollywood-movie') {
            detailType = 'movie';
        } else if (type === 'bollywood-tv') {
            detailType = 'tv';
        }
        window.location.href = `details.html?id=${id}&type=${detailType}`;
    }
